%% In this script, we confirm that our implementation of information limiting correlations
% causes information to saturate as the number of neurons in the population
% increases
...
% we'll iterate through several population sizes and pick a random subset
% of the neurons from each area
numNeuronsList = [10 50 100 200 400 600 800 1000];

% open figure
figure; hold on;
set(gcf,'Position',[10 10 700 700]);
title('confirming information limiting correlations cause total info to saturate');
...
for a = 1:length(areas)
    ...
    % for each simulated population size
    for n = 1:numel(numNeuronsList)

        % how many neurons in this area?
        numNeuronsActual = size(area.allresp,1);

        % how many neurons in this area?
        numNeurons = numNeuronsList(n);

        % make identity matrix
        C_identity_corr = eye(numNeurons);

        % make a list of random neuron indices
        neuronInds = randsample(numNeuronsActual,numNeurons,'true');

        % clear out identity matrix
        C_identity = [];
        ...
        for i = 1:numNeurons
            ...
            for d = 1:length(cntr_disp)
                C_identity(i,i,d) = area.est_variances_clean(neuronInds(i),d);
            end
        end
        ...
        for d = 1:length(cntr_disp)
            thisC = C_identity(:,:,d);
            invC = inv(thisC);
            FI_gauss_identity(d) = area.first_derivs(neuronInds,d)'*invC*area.first_derivs(neuronInds,d);
        end
        FI_gauss_identity_total(n) = sum(FI_gauss_identity);
        ...
        for d = 1:length(cntr_disp)
            ...
            C_info = area.first_derivs(neuronInds,d)*area.first_derivs(neuronInds,d)';

            % apply strongest info limiting scale factor for this area (2x)
            C_info = C_info * area.scale_factors(end);

            % add these information limiting correlations to the
            % identity covariance matrix, while keeping the diagonal unchanged
            C_info(C_identity_corr(:,:) == 1) = 0;

            C_info = C_identity(:,:,d) + C_info;

            % apply information limiting correlations
            invC = inv(C_info);
            FI_gauss_info(d) = area.first_derivs(neuronInds,d)'*invC*area.first_derivs(neuronInds,d);

        end

        % how did these correlations affect total information?
        FI_gauss_info_total(n) = sum(FI_gauss_info);
    ...
    switch this_area
        case 'V1'
            subplot(3,1,1); hold on; title('V1')
            plot(numNeuronsList,FI_gauss_identity_total,'o-','color',ColorIt('b'),'markerfacecolor',ColorIt('b'));
            plot(numNeuronsList,FI_gauss_info_total,'s--','color',ColorIt('b'),'markerfacecolor',ColorIt('b'));
        case 'V2'
            subplot(3,1,2); hold on; title('V2')
            plot(numNeuronsList,FI_gauss_identity_total,'o-','color',ColorIt('g'),'markerfacecolor',ColorIt('g'));
            plot(numNeuronsList,FI_gauss_info_total,'s--','color',ColorIt('g'),'markerfacecolor',ColorIt('g'));
        case 'MT'
            subplot(3,1,3); hold on; title('MT')
            plot(numNeuronsList,FI_gauss_identity_total,'o-','color',ColorIt('r'),'markerfacecolor',ColorIt('r'));
            plot(numNeuronsList,FI_gauss_info_total,'s--','color',ColorIt('r'),'markerfacecolor',ColorIt('r'));
    end

    xlabel('number of neurons in simulated population')
    ylabel('total Fisher Information')
    saveas(gcf,['./plots/NoiseCorrelations/TotalInformation_saturationWithIncreasingN.png']);
